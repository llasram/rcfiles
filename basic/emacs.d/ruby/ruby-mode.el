;;; ruby-mode.el --- Major mode for editing Ruby scripts

;; Copyright (C) 1994-2005  Yukihiro Matsumoto

;; Author: Yukihiro Matsumoto <matz@netlab.jp>
;; 	(who else?)
...
(defconst ruby-matching-delimiters-alist
  '((?{ . ?}) (?( . ?)) (?[ . ?]) (?< . ?>)))

(defmacro ruby-make-keywords-regexp (&rest keywords)
  (let (words)
    `(concat "\\_<\\(?:"
            (regexp-opt (append ,@keywords))
            "\\)\\_>")))
...
(defconst ruby-block-beg-re
  (ruby-make-keywords-regexp ruby-block-beg-keywords))

(defconst ruby-non-block-do-keywords
  '("while" "until" "for" "rescue"))

(defconst ruby-non-block-do-re
  (ruby-make-keywords-regexp ruby-non-block-do-keywords))

(defconst ruby-indent-beg-keywords
  '("class" "module" "def"
    "if" "unless" "case" "while" "until" "for" "begin"))

(defconst ruby-indent-beg-re
  (concat "^\\s-*" (ruby-make-keywords-regexp ruby-indent-beg-keywords)))

(defconst ruby-modifier-beg-keywords
  '("if" "unless" "while" "until"))

(defconst ruby-modifier-beg-re
  (ruby-make-keywords-regexp ruby-modifier-beg-keywords))

(defconst ruby-modifier-keywords
  (append ruby-modifier-beg-keywords '("rescue")))

(defconst ruby-modifier-re
  (ruby-make-keywords-regexp ruby-modifier-keywords))

(defconst ruby-block-mid-keywords
  '("then" "else" "elsif" "when" "rescue" "ensure"))

(defconst ruby-block-mid-re
  (ruby-make-keywords-regexp ruby-block-mid-keywords))

(defconst ruby-block-op-keywords
  '("and" "or" "not"))

(defconst ruby-block-op-re
  (ruby-make-keywords-regexp ruby-block-op-keywords))

(defconst ruby-block-hanging-re
  (ruby-make-keywords-regexp ruby-block-op-keywords
                             ruby-modifier-beg-keywords))

(defconst ruby-block-end-keywords
  '("end"))

(defconst ruby-block-end-re
  (ruby-make-keywords-regexp ruby-block-end-keywords))
...
(defconst ruby-delimiter-re
  (concat "[][?$/%(){}#\"'`.:]\\|"
          (ruby-make-keywords-regexp ruby-block-beg-keywords
                                     ruby-block-end-keywords) "\\|"
          "^=begin\\|"
          ruby-here-doc-beg-re))

(defconst ruby-negative-re
  (concat "^[ \t]*"
          "\\(?:" (ruby-make-keywords-regexp ruby-block-mid-keywords
                                             ruby-block-end-keywords) "\\|"
                  "[\]}]"
...
;;;###autoload
(define-derived-mode ruby-mode nil "Ruby"
  "Major mode for editing Ruby scripts.
\\[indent-for-tab-command] properly indents subexpressions of
multi-line class, module, def, if, while, for, do, and case
statements, taking nesting into account.

The variable `ruby-indent-level' controls the amount of
indentation.
\\{ruby-mode-map}"
...
(defun ruby-forward-string (opening &optional end no-error expand)
  (let* ((closing (or (cdr (assoc opening ruby-matching-delimiters-alist))
                      opening))
         (re (concat "[^\\]\\(?:\\\\\\\\\\)*\\(?:"
                     (regexp-quote (char-to-string opening)) "\\|"
                     (regexp-quote (char-to-string closing))
                     (when expand (concat "\\|\\(#{\\)"))
                     "\\)"))
         (n 1))
    (while (and (re-search-forward re end no-error)
		(if (match-beginning 1)
		  ...
		  (< 0 (setq n (if (eq (char-before) closing) (1- n) (1+ n))))))
      (forward-char -1))
    (cond ((zerop n))
	  (no-error nil)
	  ((error "unterminated string")))))

(defun ruby-deep-indent-paren-p (char)
  (cond ((listp ruby-deep-indent-paren)
	 (let ((deep (assoc char ruby-deep-indent-paren)))
	   (cond (deep
		  (or (cdr deep) ruby-deep-indent-paren-style))
		 ((memq char ruby-deep-indent-paren)
		  ruby-deep-indent-paren-style))))
	((eq char ruby-deep-indent-paren) ruby-deep-indent-paren-style)
	((eq char ?\( ) ruby-deep-arglist)))
...
(defun ruby-parse-partial (&optional end in-string nest depth pcol indent)
  ...
  (when (re-search-forward ruby-delimiter-re end 'move)
    (let ((pnt (point)) w re expand)
      (goto-char (match-beginning 0))
      (cond
       ((looking-at "[\"`']")
        (let ((quote (char-after)))
          (unless (ruby-forward-string quote end t (not (eq quote ?')))
            (setq in-string (point))
            (goto-char end))))
       ((looking-at "/")
	(if (ruby-expr-beg 'expr-re)
            (unless (ruby-forward-string ?/ end t t)
              (setq in-string (point))
              (goto-char end))
	  (goto-char pnt)))
       ((looking-at "%")
	(cond
	 ((and (ruby-expr-beg 'expr-qstr)
	       (not (looking-at "%="))
	       (looking-at "%[QqrxWw]?\\([^a-zA-Z0-9 \t\n]\\)"))
	  (goto-char (match-beginning 1))
	  (setq expand (not (memq (char-before) '(?q ?w))))
	  (setq w (match-string 1))
	  (unless (or (ruby-forward-string (char-after) end t expand)
                      (re-search-forward (if (string= w "\\")
                                             "\\\\[^\\]*\\\\"
                                           (concat "[^\\]\\(\\\\\\\\\\)*" w))
                                         end t))
	    (setq in-string (point))
	...
	(cond
	 ((and (ruby-expr-beg 'heredoc)
	  ...
	  (setq re (ruby-here-doc-end-match))
	  (let* ((id-end (goto-char (match-end 0)))
	    ...
	    (if (re-search-forward re end 'move)
		(forward-line 1)
  ...
  (save-excursion
	...
	(cond
	 ((and (not (ruby-deep-indent-paren-p paren))
	       (re-search-forward ruby-negative-re eol t)
               (not (eq ?_ (char-after (match-end 0)))))
...
(defun ruby-beginning-of-indent ()
  (and (re-search-backward ruby-indent-beg-re nil 'move)
    ...
    (let ((i (or cnt 1)))
	  ...
	  (while (> i 0)
	    ...
	    (cond ((looking-at "\\s)")
		   ...
		   nil)
		  ((looking-at "\\s\"\\|\\\\\\S_")
		   (let ((char (char-to-string (char-before (match-end 0)))))
		     (while (and (search-backward char)
